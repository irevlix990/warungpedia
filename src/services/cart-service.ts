...
export const getOrderById = cache(
  async (orderId: string): Promise<Order | null> => {
    ...
    return {
      ...
      paymentMethod: ((order as Record<string, unknown>).payment_method as string) === 'COD' ? 'COD' : ((order as Record<string, unknown>).payment_method as string) === 'WALLET' ? 'WALLET' : 'BANK_TRANSFER',
