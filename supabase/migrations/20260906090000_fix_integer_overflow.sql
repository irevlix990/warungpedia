-- Migration 20260906090000_fix_integer_overflow.sql
-- Fix "Integer out of range" (22003) in pay_order.
-- PostgreSQL integer is 32-bit (max ~2.1B). When computing:
--   v_gross (integer) * v_bps (integer)
-- if v_gross >= 4,294,968 (≈Rp4,3 juta) and v_bps = 500 (5%),
-- the intermediate product exceeds 2^31 - 1, causing 22003.
...
begin
  ...
  loop
    ...
    -- Cast to bigint BEFORE multiplication to avoid integer overflow
